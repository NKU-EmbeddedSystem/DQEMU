...
B<x-block-latency-histogram-set>  (Command)
Manage read, write and flush latency histograms for the device.

If only C<device> parameter is specified, remove all present latency histograms
...
=item C<x_rd_latency_histogram: BlockLatencyHistogramInfo> (optional)

C<BlockLatencyHistogramInfo>. (Since 2.12)

=item C<x_wr_latency_histogram: BlockLatencyHistogramInfo> (optional)

C<BlockLatencyHistogramInfo>. (Since 2.12)

=item C<x_flush_latency_histogram: BlockLatencyHistogramInfo> (optional)
...
finished its work, waiting for C<block-job-finalize>.
...
has completed ceased all work, and wait for C<block-job-dismiss>.
...
=item C<x-disabled: boolean> (optional)
...
=item C<dst_name: string>

name of the destination dirty bitmap

=item C<src_name: string>
...
B<x-block-dirty-bitmap-enable>  (Command)
...
B<x-block-dirty-bitmap-disable>  (Command)
...
B<x-block-dirty-bitmap-merge>  (Command)
Merge C<src_name> dirty bitmap to C<dst_name> dirty bitmap. C<src_name> dirty
bitmap is unchanged. On error, C<dst_name> is unchanged.

B<Returns:>
nothing on success
If C<node> is not a valid block device, DeviceNotFound
If C<dst_name> or C<src_name> is not found, GenericError
...
=item The members of C<BlockdevOptionsReplication> when C<driver> is "replication"
...
C<preallocation>    Preallocation mode (allowed values: off, full)
...
C<file>             Node to create the image format on
C<size>             Size of the virtual disk in bytes
C<preallocation>    Preallocation mode for the new image (allowed values: off,
...
B<x-nbd-server-add-bitmap>  (Command)
...
=item C<bitmap: string>

Bitmap name to search for.

=item C<bitmap-export-name: string> (optional)
...
B<Note:>
the client must use NBD_OPT_SET_META_CONTEXT with a query of
"qemu:dirty-bitmap:NAME" (where NAME matches C<bitmap-export-name>) to access
the exposed bitmap.
...
=item C<data: ChardevSpiceChannel> when C<type> is "spicevmc"


=item C<data: ChardevSpicePort> when C<type> is "spiceport"
...
=item C<x-multifd-channels: int> (optional)
...
=item C<x-multifd-page-count: int> (optional)
...
C<x-block-dirty-bitmap-enable>: since 3.0

=item -

C<x-block-dirty-bitmap-disable>: since 3.0
...
One of "abort", "block-dirty-bitmap-add", "block-dirty-bitmap-clear", "x-block-dirty-bitmap-enable", "x-block-dirty-bitmap-disable", "blockdev-backup", "blockdev-snapshot", "blockdev-snapshot-internal-sync", "blockdev-snapshot-sync", "drive-backup"
...
=item C<data: BlockDirtyBitmap> when C<type> is "x-block-dirty-bitmap-enable"


=item C<data: BlockDirtyBitmap> when C<type> is "x-block-dirty-bitmap-disable"
...
=item C<reason: CommandDropReason>
